...
export default function Stoklar() {
  ...
  const fetchData = async () => {
    try {
      ...
      const hMapped = (hData || []).map((h: any) => ({
        ...
        birim: h.birim,
        birim_fiyat: parseFloat(h.alis_fiyati || '0'), 
  ...
  const handleHammaddeEntry = async () => {
    ...
    const selected = hammaddeler.find(h => h.id === Number(hammaddeForm.id) || h.id === hammaddeForm.id); 
    if (!selected) {
    ...
    try {
      ...
      const { error } = await supabase
        .from("hammadde")
        .update({ kalan_miktar: String(yeniStok) }) 
    ...
    try {
      ...
      if (currentStock) {
        ...
        const { error: insertError } = await supabase
          .from("urun_stok")
          .insert({
            urun_stok_id: Math.floor(Date.now() / 1000), 
            urun_id: selected.id,
            miktar: yeniStok,
